  ...
  User.findOne({ _id: req.user })
    .then((user) => {
      if (user.google_auth) {
        return res.status(403).json({
          error:
            "You can't change accounts password  beacause you login through google",
        });
      }
          ...
          bcrypt.hash(newPassword, 10, (err, hashed_password) => {
            User.findOneAndUpdate(
              ...
              .catch((err) => {
                return res.status(500).json({
                  error:
                    "Some error occured while saving new pasword, please try again later",
                });
              });
          });
        }
    ...
    .then((comment) => {
      ...
      Blog.findOneAndUpdate(
        { _id: comment.blog_id },
        {
          ...
          "activity.total_parent_comment": comment.parent ? 0 : -1,
        }
      ).then((blog) => {
